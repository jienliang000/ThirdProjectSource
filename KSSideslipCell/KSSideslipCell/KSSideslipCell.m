...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    if ([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        // 询问代理是否需要侧滑
        if ([_ksdelegate respondsToSelector:@selector(sideslipCell:canSideslipRowAtIndexPath:)]) {
            shouldBegin = [_ksdelegate sideslipCell:self canSideslipRowAtIndexPath:self.indexPath] || _sideslip;
        }
        
        if (shouldBegin) {
            // 向代理获取侧滑展示内容数组
            if ([_ksdelegate respondsToSelector:@selector(sideslipCell:editActionsForRowAtIndexPath:)]) {
                NSArray <KSSideslipCellAction*> *actions = [_ksdelegate sideslipCell:self editActionsForRowAtIndexPath:self.indexPath];
...
- (void)actionBtnDidClicked:(UIButton *)btn {
    ...
    if ([self.ksdelegate respondsToSelector:@selector(sideslipCell:rowAtIndexPath:didSelectedAtIndex:)]) {
        _nextShowView = [self.ksdelegate sideslipCell:self rowAtIndexPath:self.indexPath didSelectedAtIndex:btn.tag];
